...
export default function Home() {
  ...
  // Auto generate initial report when page loads
  useEffect(() => {
    runAnalysis(selectedStock, uploadedFiles);
  }, []);

  const handleSelectStock = (stock: StockMarketData) => {
    setSelectedStock(stock);
    runAnalysis(stock, uploadedFiles);
  };
      ...
      <main className="mx-auto max-w-7xl px-4 pt-6 sm:px-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between rounded-2xl border border-sky-500/20 bg-gradient-to-r from-sky-950/40 via-gray-900 to-emerald-950/40 p-4 shadow-xl gap-3">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Gemini 1.5 Pro AI Analysis Engine</h3>
              <p className="text-xs text-gray-400">
                Tự động bóc tách tài liệu & lập báo cáo 4 phần A-B-C-D cho <span className="font-bold text-sky-400">{selectedStock.ticker}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3 w-full sm:w-auto">
            <button
              onClick={() => runAnalysis(selectedStock, uploadedFiles)}
        ...
        {/* Interactive Scenario Valuation Calculator */}
        {report && (
          <ValuationCalculator
